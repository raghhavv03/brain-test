...
// Practice: same 80/20 ratio and fixed-count shuffle, just fewer trials.
const PRACTICE_GO_COUNT = 8;
const PRACTICE_NO_GO_COUNT = 2;
...
export function GatekeeperGame({
  mode = "scored",
  runId = null,
  onComplete,
}: GameProps) {
  ...
  const totalTrials = isPractice
    ? PRACTICE_GO_COUNT + PRACTICE_NO_GO_COUNT
    : TOTAL_TRIALS;
  ...
  async function runTask() {
    ...
    const sequence = isPractice
      ? generateTrialSequence(PRACTICE_GO_COUNT, PRACTICE_NO_GO_COUNT)
      : generateTrialSequence();
      ...
      >
        <div className="flex flex-col items-center gap-1 text-center">
          ...
          <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground">
            {isPractice ? "Practice — not scored" : "Authorization Gate"}
          </p>
        </div>
        ...
        {phase === "idle" && (
          <div className="flex flex-col items-center gap-4 text-center">
            <p className="max-w-xs text-sm text-muted-foreground">
              ...
              do nothing. {totalTrials} signals.
            </p>
