...
export default function LockOnPage() {
  const [phase, setPhase] = useState<Phase>("idle");
  const [statusLine, setStatusLine] = useState("");
  const [roundK, setRoundK] = useState(START_K);
  ...
  async function runGame() {
    ...
    try {
      ...
      let esc: EscalationState = { k: START_K, consecutiveDiscards: 0 };
    ...
    <main className="flex min-h-screen flex-col items-center gap-4 p-6">
      ...
      {phase === "idle" && (
        <div className="flex flex-col items-center gap-2 text-center">
          <p>
            {numObjects(START_K)} dots appear; {START_K} are marked. When the
            marks vanish and the dots scatter, keep tracking them. When motion
            stops, tap the ones that were marked. Each level adds more targets
            — and more dots.
          </p>
