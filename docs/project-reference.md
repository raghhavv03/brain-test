...
Home, The Science, and About are built (Phase 4.2 — see §8a). Product, Privacy & Disclaimer, and Content / Blog remain deferred.
...
- **Shell↔lab transition:** built for the Home→/test entry direction — `src/components/shell/enter-lab-button.tsx` plays a ~400ms radial `clip-path` sweep of lab-dark from the click point, portaled to `<body>`, then navigates; reduced motion skips the sweep entirely (native `<Link>` navigation). The **results→shell exit direction is not yet built** — see §10.
...
| 4 — Polish + PWA | Feels pro, works on phones | In progress — game-skin batch complete (Echo, Circuit, Lock-On skinned and verified against live data, matching Trigger/Gatekeeper's visual language; see §9d). **Shell pages: the three core shells are done** — Home (incl. the placeholder hero + shell→lab entry sweep), Science, and About all built, typechecked, build-verified, and click-through-verified per the Phase 4.2 decisions in §8a. Remaining Phase 4 scope: Product, Privacy & Disclaimer, and Content/Blog pages (deferred per the session scope split); a full responsive polish pass; the PWA manifest; and the **results-screen→shell exit transition** (only the Home→/test entry half of the shell↔lab transition exists so far — see §8a) |
| 5 — Integration + handoff | Live + connected | Not started |

**Post-launch production incident (outside the phase sequence above).** A session-integrity bug — silent, total trial-save loss on iOS Safari — was found via a real user report and fixed between build phases, after the Phase 4.2 shell-page work. It isn't phase-scoped work in the build-order sense (every affected piece — the measurement engine, the games, the sequence wrapper — was already "Done" per the table above); it's recorded here because the fix added a genuinely new architectural layer (self-healing, run-active-gated session identity handling) to already-shipped code. Full investigation trail, fix design, and verification: §9e.
